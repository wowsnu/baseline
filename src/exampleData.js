...
// 세 번째 값이 있는 컷에만 그림이 붙는다. 나머지는 빈 카드로 둔다 —
// 하나의 그림을 여러 컷에 돌려 쓰면 서로 다른 컷이 같은 그림이 되어,
// 스토리보드가 이어지는 것처럼 보이지 않는다. 실제로 그려 둔 일곱 장만
// 각자 맞는 컷 하나에 붙인다.
...
export const EXAMPLE_SCENES = [
  { id: 'example-lab', title: '물리학과 실험실, 밤', sourceText: EXAMPLE_STORY.split('\n\n').slice(0, 2).join(' '), facts: { location: '물리학과 실험실', time: '밤' }, location: { name: '물리학과 실험실', description: '좁고 낡은 대학 실험실. 천장 형광등 하나만 살아 있어 실험대 한쪽에만 빛이 떨어진다. 오실로스코프와 뒤엉킨 케이블, 비커, 출력물이 실험대를 메우고 있다. 창밖에는 비.', image: labWide }, shots: shots('example-lab', [
    ...
    ['지친 시선', '하린이 연필을 내려놓고 의자에 등을 기댄다. 지친 얼굴로 천장을 본다.'],
    ['간격을 짚는 손', '시선이 다시 화면으로 내려온다. 어긋난 봉우리들의 간격을 눈으로 짚어 나간다. 손가락이 화면 위를 따라 움직인다.'],
    ['규칙의 발견', '그녀의 손이 멈춘다. 간격이 일정하다. 오차가 아니라 규칙이다.', labFormula],
    ['새 식', '하린이 노트를 끌어당겨 새 줄에 짧은 식 하나를 적는다. 연필 끝이 종이를 누른다.'],
    ['동그라미 친 식', '그녀가 그 식을 동그라미로 감싼다. 한 번, 두 번, 세 번. 흑연이 종이를 눌러 자국이 팬다.'],
    ['남은 시도들', '주변에는 지우개 자국과 그어 지운 시도들이 어지럽게 흩어져 있다. 그 한가운데에 방금 적은 식만 또렷하다.'],
    ['깨달음', '하린이 고개를 든다. 화면 불빛이 아래에서 얼굴을 비춘다. 눈이 화면을 지나 먼 곳에 머문다. 입술이 살짝 벌어진다.', labDiscovery],
    ['정지', '그녀는 움직이지 않는다. 형광등이 한 번 깜빡인다.'],
